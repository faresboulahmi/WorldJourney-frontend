      ...
      <div className="flex flex-row flex-wrap gap-4 p-2 m-auto min-h-[60vh]">
        {wishList.length > 0 && currentUser ? (
          <div className="flex flex-col w-full m-auto ">
            ...
            <button
              className="p-2 border-2 border-red-500 rounded-lg mx-auto my-8 text-red-500 hover:bg-red-500 hover:text-white font-semibold transition-all max-h-14 "
              onClick={() => swalDelete()}
              aria-label="delete all favorites"
            >
              Delete all tour from wishList
            </button>
          </div>
        ) : (
          currentUser &&
          !wishList.length && (
            <div className="text-2xl text-slate-700 font-semibold m-auto my-20">
              You don't have any tour your wishList{" "}
            </div>
          )
        )}
      </div>
